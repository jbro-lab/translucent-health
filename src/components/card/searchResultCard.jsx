      ...
      <Card sx={{ display: 'flex' }}>
        <CardContent sx={{ flex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Button variant="contained" onClick={handleCardClick} sx={{ flexShrink: 0 }}>
          ...
          <Collapse in={expanded} timeout="auto" unmountOnExit>
            <Typography variant="subtitle1" color="text.secondary">
              Gross Charge to Insurance: ${procedure.gross_charge ? procedure.gross_charge.toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') : null}
            </Typography>
          </Collapse>
          <Typography variant="h6" color={comparisonPrice && comparisonPrice > procedure.cash_price ? 'green': 'black'}>
            Cash Price: ${procedure.cash_price ? procedure.cash_price.toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') : null}
          </Typography>
          ...
          {procedure.distance ? <Typography variant="subtitle1" color="text.secondary" sx={{ textTransform: 'none' }}>
              {procedure.distance < 1 ? 'Less Than 1' : procedure.distance.toFixed(0)} mile{procedure.distance <= 1 ? '' : 's'}
            </Typography>: null
          }
        </CardContent>
      </Card>
