...
const ProcedurePage = () => {
  ...
  const handleInsuranceFormChange = (childState) => {
    ...
    let filteredInsuranceInfo = insuranceInfo.filter(
      (i) =>
  ...
  useEffect(() => {
    ...
    const getProcedureInfoAndComparableProcedures = async () => {
      try {
        ...
        let tempProcedureInfo = response.data?.filter(
          (d) => d["id"] === procedureId
        )[0];
          ...
          <Container>
              ...
              {isSmallScreen ? null : (
                <div className="col-3">
                  {pricePreference === "cash_price" ? (
                    <CashPriceCard
                      cashPrice={procedureInfo?.cash_price}
                      hospital={procedureInfo}
                    />
                  ) : formInformation ? (
                    <InsurancePriceCard
                      insurancePrice={filteredInsurancePrice}
                  ...
                  <Card>
                    <ToggleButtonGroup
                      ...
                      color="primary"
                    >
                      <ToggleButton value="cash_price">
                        View Cash Rate
                      </ToggleButton>
                      <ToggleButton value="insurance_price">
                        View Insurance Rate
                      </ToggleButton>
                    </ToggleButtonGroup>
                    {pricePreference === "insurance_price" ? (
            ...
            <div className="row">
              <Typography
                ...
                variant="h4"
                color="text.primary"
                sx={{ mt: 5, flex: 1, overflow: "auto" }}
              >
                <b>
                  {comparableProcedures &&
                  comparableProcedures.some(
                    (cp) => cp.cash_price < procedureInfo?.cash_price
                  )
                    ? `Save up to $${(
                        procedureInfo.cash_price -
                        comparableProcedures[0].cash_price
                      )
                        .toFixed(2)
                        .replace(
                          /(\d)(?=(\d{3})+(?!\d))/g,
                    ...
                    : `${
                        procedureInfo
                          ? "It looks like you're getting the best price!"
                          : ""
                      }`}
                </b>
              </Typography>
            ...
            <div className="row">
              {comparableProcedures && comparableProcedures.length > 0
                ? comparableProcedures.map((cp) => (
                    <SearchResults
                      key={cp.id}
                      procedure={{
                        ...cp,
